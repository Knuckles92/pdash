...
Ten module types (`markdown`, `key_value`, `table`, `timeseries`, `log_stream`, `link_list`, `iframe`, `action_button`, `notification`, `file`). Each is one file exporting Pydantic `Data` + `Config` models, registered in `modules/__init__.py:REGISTRY`. `GET /api/v1/module-schemas[/{type}]` serves their JSON Schema; the frontend's hand-rolled `SchemaForm` (`components/forms/SchemaForm.tsx`) renders edit forms from that schema. `modules.schema_version` supports lazy migrate-on-read (registry deferred). When adding/changing a module type, update the Pydantic models — the schema endpoint and frontend form follow automatically.
